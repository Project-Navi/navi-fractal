...
**navi-fractal** measures *local* mass-radius scaling: the slope of
\( \log(\text{ball mass}) \) vs \( \log(\text{radius}) \), averaged over random
...
This means the global log-ratio converges to \( d_B \) at rate \( O(1/g) \), with a
rate constant of \( \frac{\log 2}{\log u} \). Expressing this as a percentage of \( d_B \):
...
\( \text{gap}(g) = a/g + b \) per flower family, computes the amplification factor
\( |a_{\text{empirical}}| / a_{\text{theoretical}} \), and flags non-monotonic convergence. It reads
